"use strict";
/* ===== render.js — Render tampilan & cetak laporan =====
   Semua fungsi render (renderList, renderPending, renderSurat, renderAll), input suara (mic), serta ekspor Laporan PDF bulanan dan Rekap XLSX seluruh periode.
  ...
  function getMonthList(){
    ...
    return Object.keys(monthObj).map(id => ({ id, status: 'approved', ...monthObj[id] }));
  }

  // Menjumlahkan seluruh transaksi approved dari bulan-bulan SEBELUM bulan yang diberikan (saldo awal bawaan).
  ...
  function renderPending(){
    ...
    if(!list.length){ pendingListEl.innerHTML = ''; return; }
  ...
  const LETTER_LABELS = { mandat: 'Surat Tugas', undangan: 'Undangan', keterangan: 'Surat Keterangan' };
  ...
  function getLettersList(){
    return Object.keys(letters).map(id => ({ id, ...letters[id] }));
  }
  ...
  const NUMBER_WORDS = new Set([
    'nol','satu','se','dua','tiga','empat','lima','enam','tujuh','delapan','sembilan',
    'sepuluh','sebelas','belas','puluh','seratus','ratus','seribu','ribu','sejuta','juta'
  ]);
  const UNITS = {nol:0,satu:1,dua:2,tiga:3,empat:4,lima:5,enam:6,tujuh:7,delapan:8,sembilan:9};
  ...
  function wordsToNumber(tokens){
    ...
    for(const w of tokens){
      if(w === 'sepuluh'){ current += 10; continue; }
      if(w === 'sebelas'){ current += 11; continue; }
      if(w === 'seratus'){ total += 100; current = 0; continue; }
      if(w === 'seribu'){ total += 1000; current = 0; continue; }
      if(w === 'sejuta'){ total += 1000000; current = 0; continue; }
      if(w === 'se'){ current += 1; continue; }
      if(w in UNITS){ current += UNITS[w]; continue; }
      if(w === 'belas'){ current += 10; continue; }
      if(w === 'puluh'){ current = (current || 1) * 10; continue; }
      if(w === 'ratus'){ current = (current || 1) * 100; total += current; current = 0; continue; }
      if(w === 'ribu'){ current = (current || 1) * 1000; total += current; current = 0; continue; }
      if(w === 'juta'){ current = (current || 1) * 1000000; total += current; current = 0; continue; }
    }
    return total + current;
  }

  function extractAmount(lowerText){
    // 1) Coba pola angka digit dulu (Chrome sering otomatis mengubah ucapan angka jadi digit)
    ...
    if(digitMatch && digitMatch[1].replace(/[.,]/g,'').length > 0){
      ...
      if(suf === 'ribu' || suf === 'rb') val *= 1000;
      else if(suf === 'juta' || suf === 'jt') val *= 1000000;
      if(val > 0){
        return { value: val, span: digitMatch[0] };
      }
    }
    // 2) Fallback: cari deretan kata angka bahasa Indonesia
  ...
  function processVoiceInput(transcript){
    ...
    const { value: amount, span } = extractAmount(lower);
  ...
  exportBtn.addEventListener('click', ()=>{
    ...
    if(list.length === 0){ showToast('Belum ada transaksi disetujui untuk dicetak'); return; }
    ...
    function centeredBlock(text, y, fontSize, lineHeight){
      ...
      lines.forEach((line, i)=> doc.text(line, pageWidth/2, y + i*lineHeight, { align:'center' }));
    ...
    // ---- Kop / Judul (3 baris, rata tengah, tinggi dihitung otomatis) ----
    ...
    const body = list.map(t => {
      ...
      return [
        ...
        t.desc,
        t.type === 'in' ? formatAngka(t.amount) : '-',
        t.type === 'out' ? formatAngka(t.amount) : '-',
        formatAngka(runningSaldo)
      ];
    });

    // ---- Pilih ukuran font & padding tabel dengan cara mengukur hasil render asli ----
    // (bukan menebak rumus) supaya baris keterangan yang panjang & turun ke 2 baris tetap terhitung akurat.
    const sizeCandidates = [
      { fontSize: 9,   pad: 3.2 },
      { fontSize: 8.5, pad: 2.6 },
      { fontSize: 8,   pad: 2.2 },
      { fontSize: 7.5, pad: 1.8 },
      { fontSize: 7,   pad: 1.4 },
      { fontSize: 6.5, pad: 1.1 },
      { fontSize: 6,   pad: 0.9 },
      { fontSize: 5.5, pad: 0.7 },
      { fontSize: 5,   pad: 0.5 }
    ];
    const FOOTER_SPACE_NEEDED = 10 + 14 + 55; // saldo akhir + jarak + tinggi blok tanda tangan
    ...
    for(const cand of sizeCandidates){
      ...
      trialDoc.autoTable({
        head: tableHead, body: body, foot: tableFoot, startY: cursorY,
        styles: { font:'helvetica', fontSize: cand.fontSize, cellPadding: cand.pad },
        headStyles: { fillColor: [29, 99, 180], textColor: [255,255,255], halign:'center' },
        footStyles: { fillColor: [238,242,247], textColor: [22,32,43], fontStyle:'bold', halign:'right' },
        columnStyles: tableColumnStyles
      });
      const trialEnd = trialDoc.lastAutoTable.finalY;
      if(trialEnd + FOOTER_SPACE_NEEDED <= pageHeight - 10){ tableStyle = cand; break; }
    }
    ...
    doc.autoTable({
      ...
      body: body,
      startY: cursorY,
      styles: { font:'helvetica', fontSize: tableStyle.fontSize, cellPadding: tableStyle.pad },
      headStyles: { fillColor: [29, 99, 180], textColor: [255,255,255], halign:'center' },
      columnStyles: tableColumnStyles,
    ...
    doc.setFont('helvetica','normal');
    doc.setFontSize(10);
    doc.text(`Cikupa, ${lastDay} ${monthLabel(currentMonth)}`, pageWidth-14, finalY + 6, { align:'right' });
    ...
    doc.setFont('helvetica','bold');
    doc.setFontSize(10);
    doc.text('Ketua', col1X, roleY, { align:'center' });
    doc.text('Sekretaris', col2X, roleY, { align:'center' });
    doc.text('Bendahara', col3X, roleY, { align:'center' });
    ...
    doc.setFont('helvetica','bold');
    doc.setFontSize(9.5);
    doc.text(ROLE_NAMES.ketua, col1X, nameY, { align:'center' });
    doc.text(ROLE_NAMES.sekretaris, col2X, nameY, { align:'center' });
    doc.text(ROLE_NAMES.bendahara, col3X, nameY, { align:'center' });

    doc.save(`Laporan-Keuangan-${currentMonth}.pdf`);
    showToast('Laporan PDF sedang diunduh');
  });

  /* ================= EXPORT XLSX (SATU SHEET, RAPI SEPERTI PDF) ================= */
  // Beda dengan tombol PDF di atas (yang cuma mencetak 1 bulan sedang aktif),
  // tombol ini membuat 1 file Excel berisi:
  //  - sheet "Ringkasan": rekap tiap bulan berdampingan (seluruh periode)
  //  - sheet "Rekap Bulanan": SEMUA bulan digabung jadi SATU sheet, tersusun
  //    ke bawah berurutan (tinggal scroll naik-turun), tiap blok bulan
  //    meniru tata letak tabel laporan PDF (judul, saldo awal, tabel header
  //    biru, warna pemasukan/pengeluaran, baris TOTAL, saldo akhir).
  const XLSX_COLORS = { blue: '1D63B4', red: 'DC2626', grayFill: 'EEF2F7', white: 'FFFFFF', border: 'CBD5E1' };

  function xlsxBorderAll(){
    const line = { style: 'thin', color: { rgb: XLSX_COLORS.border } };
    return { top: line, bottom: line, left: line, right: line };
  }

  // Menerapkan style ke satu sel. Selnya dibuat dulu kalau belum ada isinya,
  // supaya sel kosong (mis. kolom kosong di baris judul/total) tetap bisa
  // diberi warna/border seperti tabel di PDF.
  ...
  // Menyusun baris untuk 1 bulan (meniru tata letak "Cetak Laporan PDF"),
  // TAPI tidak langsung dijadikan sheet — cuma dikembalikan sebagai baris
  // mentah + daftar style/merge yang masih relatif (dimulai dari baris 0),
  // supaya bisa "ditempel" ke sheet gabungan pada baris berapa pun nanti.
  function buildMonthBlock(monthKey){
    const list = Object.values(db[monthKey] || {})
    ...
    const rows = [];
    const ops = []; // { r, c, style } — r masih relatif terhadap awal blok ini
    const merges = []; // { s:{r,c}, e:{r,c} } — juga masih relatif
    ...
    merges.push(
      ...
      { s:{r:2,c:0}, e:{r:2,c:4} },
      { s:{r:4,c:0}, e:{r:4,c:3} },
      { s:{r:closingRowIdx,c:0}, e:{r:closingRowIdx,c:3} }
    );

    ops.push({ r:0, c:0, style:{ font:{ bold:true, sz:12 }, alignment:{ horizontal:'center' } } });
    ops.push({ r:1, c:0, style:{ font:{ bold:true, sz:10.5 }, alignment:{ horizontal:'center' } } });
    ops.push({ r:2, c:0, style:{ font:{ sz:10.5 }, alignment:{ horizontal:'center' } } });
    ops.push({ r:4, c:0, style:{ font:{ bold:true } } });
    ops.push({ r:4, c:4, style:{ font:{ bold:true }, numFmt:'#,##0', alignment:{ horizontal:'right' } } });
    ops.push({ r:closingRowIdx, c:0, style:{ font:{ bold:true, sz:12 } } });
    ops.push({ r:closingRowIdx, c:4, style:{ font:{ bold:true, sz:12 }, numFmt:'#,##0', alignment:{ horizontal:'right' } } });

    for(let c=0; c<5; c++){
      ops.push({ r: headerRowIdx, c, style:{
        font:{ bold:true, color:{ rgb: XLSX_COLORS.white } },
        fill:{ fgColor:{ rgb: XLSX_COLORS.blue } },
    ...
    for(let r=dataStartIdx; r<=dataEndIdx; r++){
      for(let c=0; c<5; c++){
        const style = { border: xlsxBorderAll() };
        if(c===2){ style.numFmt='#,##0'; style.alignment={horizontal:'right'}; style.font={ color:{ rgb: XLSX_COLORS.blue } }; }
        if(c===3){ style.numFmt='#,##0'; style.alignment={horizontal:'right'}; style.font={ color:{ rgb: XLSX_COLORS.red } }; }
        if(c===4){ style.numFmt='#,##0'; style.alignment={horizontal:'right'}; style.font={ bold:true }; }
        ops.push({ r, c, style });
      }
    }

    for(let c=0; c<5; c++){
      const style = { font:{ bold:true }, fill:{ fgColor:{ rgb: XLSX_COLORS.grayFill } }, border: xlsxBorderAll() };
      if(c>=2){ style.numFmt='#,##0'; style.alignment={horizontal:'right'}; }
      ops.push({ r: totalRowIdx, c, style });
    }

    return { rows, ops, merges };
  }
  ...
  exportXlsxBtn.addEventListener('click', ()=>{
    ...
    const allMonthKeys = Object.keys(db)
      .filter(mk => Object.values(db[mk] || {}).some(t=>t.status === 'approved'))
      .sort();
    if(allMonthKeys.length === 0){ showToast('Belum ada transaksi disetujui untuk diekspor'); return; }

    const wb = XLSX.utils.book_new();

    // ---- Sheet "Ringkasan": rekap tiap bulan berdampingan, 1 baris/bulan ----
    const ringkasanAoa = [
      ['REKAP KEUANGAN SELURUH PERIODE', '', '', '', ''],
      [],
      ['Bulan', 'Saldo Awal (Rp)', 'Pemasukan (Rp)', 'Pengeluaran (Rp)', 'Saldo Akhir (Rp)']
    ];
    ...
    allMonthKeys.forEach(mk=>{
      ...
      grandIn += inSum; grandOut += outSum; finalSaldo = closing;
    ...
    ringkasanWs['!cols'] = [ { wch:18 }, { wch:16 }, { wch:16 }, { wch:16 }, { wch:16 } ];
    ringkasanWs['!merges'] = [ { s:{r:0,c:0}, e:{r:0,c:4} } ];
    xlsxStyleCell(ringkasanWs, 0, 0, { font:{ bold:true, sz:13 }, alignment:{ horizontal:'center' } });
    for(let c=0; c<5; c++){
    ...
    for(let r=3; r<totalRingkasanRowIdx; r++){
      for(let c=0; c<5; c++){
        const style = { border: xlsxBorderAll() };
        if(c>=1){ style.numFmt='#,##0'; style.alignment={horizontal:'right'}; }
        if(c===2) style.font = { color:{ rgb: XLSX_COLORS.blue } };
        if(c===3) style.font = { color:{ rgb: XLSX_COLORS.red } };
        if(c===4) style.font = { bold:true };
    ...
    // ---- Sheet "Rekap Bulanan": semua bulan digabung jadi SATU sheet ----
    // Tiap blok bulan ditempel berurutan ke bawah, dipisah 2 baris kosong,
    // supaya tinggal scroll naik-turun tanpa pindah-pindah sheet.
    const combinedRows = [];
    const combinedMerges = [];
    const combinedOps = [];
    allMonthKeys.forEach((mk, idx)=>{
      if(idx > 0){ combinedRows.push([]); combinedRows.push([]); }
      const startRow = combinedRows.length;
      const block = buildMonthBlock(mk);
      block.rows.forEach(row => combinedRows.push(row));
      block.merges.forEach(m => combinedMerges.push({ s:{ r:m.s.r+startRow, c:m.s.c }, e:{ r:m.e.r+startRow, c:m.e.c } }));
      block.ops.forEach(op => combinedOps.push({ r: op.r+startRow, c: op.c, style: op.style }));
    });

    const bulananWs = XLSX.utils.aoa_to_sheet(combinedRows);
    bulananWs['!cols'] = [ { wch: 14 }, { wch: 42 }, { wch: 16 }, { wch: 16 }, { wch: 16 } ];
