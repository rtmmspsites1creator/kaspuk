"use strict";
/* ===== letters.js — Surat: form, approve/reject, dan cetak PDF =====
   Semua logika tab Surat: kirim/edit surat, approve/reject oleh Ketua, generator PDF tiap jenis surat (Mandat/Undangan/Keterangan).
  ...
  function collectLetterData(){
    if(selectedLetterType === 'mandat'){
      ...
      if(!nama || !jabatan || !keperluan || !mulai){ showToast('Lengkapi nama, jabatan, uraian tugas, dan tanggal mulai'); return null; }
      return { penerimaNama: nama, penerimaJabatan: jabatan, perihal, keperluan, hari, jam, tempatTugas: tempat, tanggalMulai: mulai, tanggalSelesai: selesai || mulai, catatan };
    }
    if(selectedLetterType === 'undangan'){
      ...
      if(!perihal || !tujuan || !tglAcara || !tempat){ showToast('Lengkapi perihal, tujuan, tanggal, dan tempat acara'); return null; }
      return { perihal, tujuanPeserta: tujuan, tanggalAcara: tglAcara, waktuAcara: waktu, tempatAcara: tempat, agenda, catatan };
    }
    // keterangan
    ...
    if(!namaK || !jabatanK || !isi){ showToast('Lengkapi nama, jabatan/NIK, dan isi keterangan'); return null; }
    return { namaDiterangkan: namaK, jabatanNIK: jabatanK, isiKeterangan: isi, masaBerlaku: berlaku };
  }

  function clearLetterForm(){
    ['mandatNama','mandatJabatan','mandatPerihal','mandatKeperluan','mandatHari','mandatJam','mandatTempat','mandatMulai','mandatSelesai','mandatCatatan',
     'undPerihal','undTujuan','undTanggalAcara','undWaktu','undTempat','undAgenda','undCatatan',
     'ketNama','ketJabatan','ketIsi','ketBerlaku'].forEach(id=>{
  ...
  suratSubmitBtn.addEventListener('click', ()=>{
    if(editingLetterId){
      ...
      if(!allowed){ showToast('Tidak punya izin mengedit surat ini'); return; }
    ...
    if(currentRole !== 'sekretaris' && currentRole !== 'ketua'){ showToast('Hanya Sekretaris/Ketua yang bisa membuat surat'); return; }
    const data = collectLetterData();
    if(!data) return;
  ...
  function startEditLetter(id){
    const l = letters[id];
    if(!l){ showToast('Surat tidak ditemukan'); return; }
    switchTab('surat');
    ...
    suratFormCard.scrollIntoView({ behavior:'smooth', block:'start' });
  }
  ...
  function approveLetter(id){
    ...
    }).then(()=> showToast('Surat disetujui & diberi nomor'))
      .catch(()=> showToast('Gagal menyetujui surat'));
  }

  function rejectLetter(id){
    if(currentRole !== 'ketua') return;
    fdb.ref('letters/' + id).remove()
      .then(()=> showToast('Surat ditolak & dihapus'))
  ...
  function deleteLetter(id){
    ...
    fdb.ref('letters/' + id).remove()
      .then(()=> showToast('Surat dihapus'))
      .catch(()=> showToast('Gagal menghapus, tidak punya izin'));
  }

  /* ---------------- Surat: generator PDF ---------------- */
  const LETTER_CODES = { mandat: 'ST', undangan: 'UND', keterangan: 'SKET' };

  function drawLetterhead(doc, pageWidth){
    const logoSize = 18;
    if(LOGO_DATA_URI){
      try{ doc.addImage(LOGO_DATA_URI, 'JPEG', 14, 8, logoSize, logoSize); }catch(e){}
    }
    let y = 13;
    doc.setFont('helvetica','bold'); doc.setFontSize(13);
    doc.text('PIMPINAN UNIT KERJA', pageWidth/2, y, { align:'center' }); y += 5.5;
    doc.setFontSize(10.5);
    doc.text('SERIKAT PEKERJA ROKOK TEMBAKAU MAKANAN MINUMAN', pageWidth/2, y, { align:'center' }); y += 4.8;
    doc.text('SERIKAT PEKERJA SELURUH INDONESIA', pageWidth/2, y, { align:'center' }); y += 4.8;
    doc.setFont('helvetica','bold');
    doc.text('( PUK SP RTMM - SPSI PT TORABIKA EKA SEMESTA 1 )', pageWidth/2, y, { align:'center' }); y += 4.6;
    doc.setFont('helvetica','normal'); doc.setFontSize(8);
    doc.text('NOMOR BUKTI PENCATATAN DISNAKERTRANS NOMOR: 243/Disnakertrans/X1/2001', pageWidth/2, y, { align:'center' }); y += 3.2;
    doc.setDrawColor(30,30,30); doc.setLineWidth(0.2);
    doc.line(16, y, pageWidth-16, y); y += 4;
    doc.setFont('helvetica','bold'); doc.setFontSize(9);
    doc.text('Jln. Raya Serang KM. 12,5 Kecamatan Cikupa Kabupaten Tangerang', pageWidth/2, y, { align:'center' }); y += 3;
    doc.setDrawColor(29,99,180); doc.setLineWidth(0.9);
    doc.line(14, y, pageWidth-14, y); y += 0.9;
    doc.setDrawColor(30,30,30); doc.setLineWidth(0.2);
  ...
  // Menggambar blok penutup surat: Dikeluarkan di/Pada Tanggal, nama organisasi, kolom TTD, dan Tembusan.
  // signers: array of { role, name }, 1-2 kolom.
  function drawLetterClosing(doc, pageWidth, marginX, y, tanggalSurat, signers){
    const pageHeight = doc.internal.pageSize.getHeight();
    const estimatedHeight = 65; // perkiraan tinggi blok penutup (tanggal + nama organisasi + TTD + tembusan)
    if(y + estimatedHeight > pageHeight - 12){
      doc.addPage();
      y = 20;
    }
    doc.setFont('helvetica','normal'); doc.setFontSize(10);
    doc.text('Dikeluarkan di  : Cikupa', pageWidth-marginX, y, { align:'right' }); y += 5.5;
    doc.text('Pada Tanggal    : ' + fmtTgl(tanggalSurat), pageWidth-marginX, y, { align:'right' }); y += 14;

    doc.setFont('helvetica','bold'); doc.setFontSize(10);
    doc.text('PIMPINAN UNIT KERJA', pageWidth/2, y, { align:'center' }); y += 4.6;
    doc.setFontSize(8.5);
    doc.text('SERIKAT PEKERJA - ROKOK, TEMBAKAU, MAKANAN DAN MINUMAN', pageWidth/2, y, { align:'center' }); y += 4.2;
    doc.text('SERIKAT PEKERJA SELURUH INDONESIA', pageWidth/2, y, { align:'center' }); y += 4.2;
    doc.setFont('helvetica','bold');
    doc.text('PT TORABIKA EKA SEMESTA 1 CIKUPA', pageWidth/2, y, { align:'center' }); y += 16;
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(10);
    signers.forEach((s,i)=> doc.text(s.role + ',', centers[i], y, { align:'center' }));
    ...
    signers.forEach((s,i)=> doc.text(s.name, centers[i], nameY, { align:'center' }));

    let ty = nameY + 14;
    doc.setFont('helvetica','italic'); doc.setFontSize(8.5);
    doc.text('Tembusan :', marginX, ty); ty += 4.5;
    doc.setFont('helvetica','normal');
    doc.text('1. Kepada Yth, Pimpinan Perusahaan PT. Torabika Eka Semesta Cikupa.', marginX+4, ty); ty += 4.2;
  ...
  function downloadLetterPDF(id){
    ...
    // Baris "Label : Nilai" dengan kolon rata di posisi tetap, mendukung nilai multi-baris.
    function labelValue(label, value, fontSize, lineHeight){
      if(y > pageHeight - 20){ doc.addPage(); y = 20; }
      doc.setFont('helvetica','normal'); doc.setFontSize(fontSize || 10.5);
    ...
    if(l.type === 'mandat'){
      ...
      doc.text('Nomor : ' + (l.nomorSurat || '-'), marginX, y); y += 5.5;
      doc.text('Hal      : ' + (l.data.perihal || '-'), marginX, y); y += 10;
      ...
      if(l.data.catatan){ y += 1; para('Catatan: ' + l.data.catatan, 10, 5, false); }
      y += 4;
      ...
      drawLetterClosing(doc, pageWidth, marginX, y, l.tanggalSurat, [
        { role: 'Ketua', name: ROLE_NAMES.ketua },
        { role: 'Sekretaris', name: ROLE_NAMES.sekretaris }
      ]);

    } else if(l.type === 'undangan'){
      doc.setFont('helvetica','normal'); doc.setFontSize(10.5);
      doc.text('Nomor    : ' + (l.nomorSurat || '-'), marginX, y); y += 5.5;
      doc.text('Lampiran : -', marginX, y); y += 5.5;
      doc.text('Perihal  : ' + l.data.perihal, marginX, y); y += 10;
      ...
      doc.setFont('helvetica','normal'); doc.setFontSize(10.5);
      doc.text('Nomor : ' + (l.nomorSurat || '-'), marginX, y); y += 10;
      ...
      if(l.data.masaBerlaku){ y += 1; para('Surat keterangan ini berlaku sampai dengan ' + fmtTgl(l.data.masaBerlaku) + '.', 10.5, 5.2, false); }
      y += 4;
